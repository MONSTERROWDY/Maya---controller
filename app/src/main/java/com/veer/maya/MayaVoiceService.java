...
public class MayaVoiceService extends Service {
    ...
    @Override public void onCreate() {
        ...
        Notification n = new Notification.Builder(this,"maya_voice")
                .setContentTitle("MAYA active").setContentText("Voice assistant is running")
        ...
        tts = new TextToSpeech(this, status -> { try { tts.setLanguage(new Locale("hi","IN")); } catch(Exception ignored) {} });
    ...
    private void listen() {
        ...
        sr.setRecognitionListener(new RecognitionListener() {
            ...
            public void onError(int e){ listening=false; h.postDelayed(() -> listen(),1000); }
    ...
    private void handle(String heard) {
        ...
        String x=heard.trim(); String l=x.toLowerCase(Locale.US);
        boolean wake=l.contains("maya") || x.contains("माया");
        if (!wake) return;
        x=x.replaceAll("(?i)\\bmaya\\b"," ").replace("माया"," ").trim();
